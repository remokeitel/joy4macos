...
class ProController: DSUController {
    ...
    let availableButtons: [JoyCon.Button] = [.A, .B, .X, .Y, .L, .ZL, .R, .ZR, .Up, .Right, .Down, .Left, .Minus, .Plus, .Capture, .Home, .LStick, .RStick]
    ...
    override func updateMotionVariables() {
        ...
        accZ = DSUUtils.getUInt8arrayFromCGFloat(num:  self.gameController.acceleration.x)
